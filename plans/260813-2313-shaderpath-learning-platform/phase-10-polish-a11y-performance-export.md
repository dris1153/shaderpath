...
**✅ RESOLVED 2026-08-14** by `plans/260814-2139-bilingual-exercise-code/` (commits `c55c806`, `1f60c4d`, phase-3 lint guard). Outcome: prose answers moved to a bilingual `solutionNote` field (118 exercises), code comments standardised on English, and `lint:content` now errors on Vietnamese in `starterCode`/`solutionCode`. Original entry kept below for context.

**Backlog — bilingual code comments (raised 2026-08-14):** `Exercise.starterCode`/`solutionCode` are plain `string`, not `Localized<string>`, so the `en` locale shows Vietnamese comments in every code/shader exercise — measured **1,103 comment lines across 139 of 162 `exercises.ts` files**. Playground presets were fixed with a `// @key` marker + shared dictionary (one code body, per-locale comment text; see `content/playground-presets/comments.ts`), but applying the same mechanism to 1,103 lines means ~2,200 strings to maintain and forces every future exercise author to write markers instead of plain comments. Options if revisited: (a) translate the 1,103 comments to English in one sweep and declare "prose bilingual, code English", (b) marker-ise them, (c) leave as-is — chosen for now, since this is a single-user app whose owner reads Vietnamese.
