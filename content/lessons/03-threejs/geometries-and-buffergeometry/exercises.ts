...
Then answer: does raising $s$ from $2$ to $4$ (doubling it) double the triangle count — or more? Why?`,
    ...
    solutionCode: `// Mỗi mặt: lưới s×s ô, 2 tam giác/ô → 2s² tam giác/mặt.
// Khối có 6 mặt → triangles(s) = 6 * 2s² = 12s².
//
// triangles(2) = 12 * 2² = 12 * 4  = 48
// triangles(4) = 12 * 4² = 12 * 16 = 192
//
// 192 / 48 = 4, không phải 2. triangles(s) là hàm BẬC HAI theo s (chứa s²)
// — nhân đôi s nhân s² lên 4 lần, đúng quy luật diện tích một lưới 2D tăng
// theo bình phương khi cả hai chiều cùng tăng theo cùng tỉ lệ. Đo trực
// tiếp trên BoxGeometry xác nhận đúng: s=1→12, s=2→48, s=4→192, s=8→768.`,
  },
  {
...
function buildFlatTriangle(
  ...
  // TODO 1: tính pháp tuyến mặt bằng cross((b - a), (c - a)), normalize
  // TODO 2: đẩy 3 đỉnh vào positions (9 số)
  // TODO 3: đẩy CÙNG một normal vào normals, lặp lại cho cả 3 đỉnh (9 số)
...
function buildFlatTriangle(
  ...
  const n = ab.cross(ac).normalize(); // giống triangleNormal() ở Track 0

  const positions: number[] = [];
  const normals: number[] = [];
  for (const p of [a, b, c]) {
    positions.push(p.x, p.y, p.z);
    // Shading phẳng: cả 3 đỉnh của MỘT tam giác dùng chung đúng một normal.
